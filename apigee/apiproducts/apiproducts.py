...
class Apiproducts:
    ...
    def list_api_products(self, expand=False, count=1000, startkey="", format='json'):
    ...
    def push_apiproducts(self, file):
        ...
        console.echo(f'Updating {self._apiproduct_name} if exists')
        resp = self.update_api_product(json.dumps(apiproduct))
        if resp.status_code != 404:
            return resp
        console.echo(f'{self._apiproduct_name} did not exist, creating now')
        return self.create_api_product(json.dumps(apiproduct))
